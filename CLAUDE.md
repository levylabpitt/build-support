...
- **Release assets** are collected from `builds\latest` only. The `.vipb` writes the `.vip` to its own `Library_Output_Folder` (usually `builds\Package`), so `build.bat` stages it across - only the exact `<package>-<version>.vip` for the version being built, whose presence is also the script's test that the build worked (see Known sharp edges). Inno writes the `Setup.exe` there directly.
...
- **The app does not consume the package.** `build-support.lvproj` has 22 `../lvsrc/` references and zero `<userlib>` ones, so Patrick Builder statically links a snapshot of the source beside it. The `build-support 1.11.x` in its window title (a string constant in `Patrick Builder.vi`, `1.11.4` as of this writing) is hardcoded, not an enforced dependency. This is the substance of issue #24.
...
- **`build.bat` has two installers.** The GUI runs `%LOCALAPPDATA%\LevyLab\build-support\scripts\build.bat` (User Application Data + `LevyLab\build-support` + `scripts`, in `Patrick Builder.vi`). The Build Support package installs it there, and so does the `Patrick Builder Installer` spec in `build-support.lvproj`, which ships `lvsrc/Scripts/build.bat` and `Setup-BuildMachine.bat` to the same folder. Whichever installer ran last wins, so an older Patrick Builder install silently downgrades `build.bat`; uninstalling Patrick Builder (MSI) probably deletes it; and MSI may skip overwriting a VIPM-written copy. The same spec still lists old `*.template` files with empty source tags. Issue #37 proposes making the package the only owner.
...
- **Publishing to the package repository is Patrick Builder's job, not `build.bat`'s.** `Publish Packages to Package Repository.vi` (in `Build.lvlib`, via the VIPM API) writes the `.vip`, `.spec` and `.bmp` into the `levylabpitt.github.io` clone and commits them - every publish commit there reads `Patrick Builder publish packages: <package>-<version>`. `build.bat` has no publish step in any version. A release driven from the command line therefore reaches GitHub but never the VIPM repository. Issue #32 tracks the fix: extract the publish core into a subVI shared by the GUI and a g-cli tool, driven by a `PUBLISH_REPO=` config key (empty by default).
...
- **g-cli's exit code does not prove a VIP build worked.** g-cli exits 0 when it loses its connection to LabVIEW mid-build (`Comms Error: Unexpected EOF`). `build.bat` used to trust that, then stage the newest `.vip` in the output folder - an old one - and release it under the new version: Transport 2.4.3.51 and 2.4.3.52 both shipped the 2.4.2.50 package. It now moves any existing `*-<version>.vip` aside before building and fails unless that exact file exists afterwards.
- **Do not force-close LabVIEW before a build.** `taskkill /F` is a crash as far as LabVIEW is concerned: its next launch - the g-cli one - opens into the "LabVIEW did not shut down properly" recovery prompt, which is the likely cause of the mid-build connection loss above. Saving everything first does not help. `:close_labview` sends a normal close request and forces only after `QUIT_TIMEOUT` seconds (default 90, overridable in a config). Measured on LabVIEW 2019: about 15 s for a warm instance, over 30 s for a freshly launched one. `g-cli QuitLabVIEW` also exits cleanly, but targets only one LabVIEW version. For the same reason, do not use g-cli's `--kill`.
- **A LabVIEW left open after an Application Builder build has dead file dialogs.** File»Open does nothing and new VIs will not save (existing VIs still save, since they skip the dialog) until LabVIEW restarts. The builder moves the process working directory into the build output folder, the build clears and recreates that folder, and the common dialog opens at a directory that no longer exists. g-cli leaves the instance running, so `build.bat` calls `:close_labview` again after the last LabVIEW step (and on failure). The alternatives, `allowmultipleinstances=True` to keep builds out of the interactive instance, or `useNativeFileDialogs=False` as a backstop, change LabVIEW's INI globally and are not used.
